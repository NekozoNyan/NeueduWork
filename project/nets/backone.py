...
class Backbone(nn.Module):
    def __init__(self, base_channels, base_depth, deep_mul, phi, pretrained=False):
        ...
        if pretrained:
            ...
            print(f"读取权重 来自{url.split('/')[-1]}")

    def forward(self, x):
        x = self.stem(x)
        x = self.dark2(x)
        # ---------------------------------
        # dark3 输出为256，80，80，是一个有效特征层
        # ---------------------------------
        x = self.dark3(x)
        feat1 = x
        # ---------------------------------
        # dark3 输出为512，40，40，是一个有效特征层
        # ---------------------------------
        x = self.dark4(x)
        feat2 = x

        # ---------------------------------
        # dark5 输出为1024*deep_mul,20,20，是一个有效特征层
        # ---------------------------------
        x = self.dark5(x)
        feat3 = x
