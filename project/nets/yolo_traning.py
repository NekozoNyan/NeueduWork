...
def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9, roll_out=False):
    ...
    """
    n_anchors = xy_centers.shape[
        0]  # 锚点数量 这里的[0]是一个索引，它引用了形状元组中的第一个元素 获取xy_centers张量或数组的第一个维度的大小，即行数，并将这个值赋给变量n_anchors
    ...
    if roll_out:
        ...
        for b in range(bs):
            # gt_bboxes[b]：选取批次 b 中的所有真实框。gt_bboxes 的形状是 (b, n_boxes, 4)，其中 b 是批次大小，n_boxes 是每个批次中的真实框数量，4 表示每个真实框的坐标（通常是 [x1, y1, x2, y2] 或 [center_x, center_y, width, height]）
            # .view(-1, 1, 4)：这是一个重塑（reshape）操作。-1 表示自动计算该维度的大小，以保持元素总数不变。1 表示将第二个维度重塑为 1，4 表示保持每个真实框的坐标维度为 4。重塑后的形状可能是 (n_boxes, 1, 4)，这取决于 gt_bboxes[b] 的原始形状。
            # .chunk(2, 2)：这是一个分割（chunk）操作。它将重塑后的张量沿着指定的维度（这里是第二个维度，即维度索引为 1 的维度，但由于前面已经重塑为 1，实际上这个操作是在最后一个维度上进行分割）分割成多个块。2 表示要分割成 2 个块，(2, 2) 中的第二个 2 表示每个块的大小（在这个例子中，每个真实框的坐标被分割成两部分，即左上角 [x1, y1] 和右下角 [x2, y2]）
            # 作用是：对于批次 b 中的每个真实框，将其坐标分割成左上角 lt（left-top）和右下角 rb（right-bottom）两部分
        ...
        # view 函数用于改变张量的形状。
        # -1 表示自动计算该维度的大小，以保持元素总数不变。
        # 1 表示将第二个维度重塑为1。
        # 4 表示保持每个真实框的坐标维度为4（ [x1, y1, x2, y2] 表示左上角和右下角的坐标）
        # .chunk(2, 2)：
        # chunk 函数用于将张量分割成多个块。
        # 第一个参数 2 表示要沿着指定的维度分割成2个块。
        # 第二个参数 2 表示每个块的大小。在这个上下文中，它实际上是指沿着最后一个维度（即坐标维度）将每个真实框的坐标分割成两部分：左上角 [x1, y1] 和右下角 [x2, y2]
        lt, rb = gt_bboxes.view(-1, 1, 4).chunk(2, 2)
        # 真实框距离每个anchors锚点的左上右下的距离
        # torch.cat 用于将一系列张量沿着指定的维度拼接起来。
        # xy_centers[None] - lt 和 rb - xy_centers[None] 是两个计算表达式，分别计算锚点中心与真实框左上角、右下角的坐标差。
        # dim=2 指定了拼接的维度，即沿着第三个维度（索引从0开始）进行拼接。
        # -1 会被替换为拼接后张量的最后一个维度的大小
        bbox_deltas = torch.cat((xy_centers[None] - lt, rb - xy_centers[None]), dim=2).view(bs, n_boxes, n_anchors, -1)
        # return (bbox_deltas.min(3)[0] > eps).to(gt_bboxes.dtype)
        # 3 指定了要在哪个维度上计算最小值。张量的维度索引是从0开始的，所以3表示在第四个维度上计算最小值。
        # gt_ 是一个逐元素比较操作，表示“greater than”（大于）。
        # eps 是一个很小的正数，用作比较的阈值。
        # gt_(eps) 会将张量中所有小于或等于eps的元素替换为False，所有大于eps的元素替换为True。这个操作是原地（in-place）的，意味着它会直接修改bbox_deltas张量，而不是返回一个新的张量
...
def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
    """如果锚框被分配给多个gts，
        将选择iou最高的那个。
        Args：
            mask_pos（张量）：形状（b，n_max_boxes，h*w）
            重叠（张量）：形状（b，n_max_boxes，h*w）
        返回：
            target_gt_idx（张量）：形状（b，h*w）
            fg_mask（张量）：形状（b，h*w）
            mask_pos（张量）：形状（b，n_max_boxes，h*w）
    """
    # b, n_max_boxes, 8400 -> b, 8400
    # 计算每个锚框是否匹配到真实框，即mask_pos中每行（对应一个锚框）的和，大于0表示匹配到。
    fg_mask = mask_pos.sum(-2)
    # 如果有一个anchor被指派去预测多个真实框
    # 如果有一个锚框被指派去预测多个真实框（即fg_mask中有大于1的值）。
    if fg_mask.max() > 1:
        # b, n_max_boxes, 8400
        # 构造一个与mask_pos形状相同的张量，用于标记哪些锚框被指派给了多个真实框。
        mask_multi_gts = (fg_mask.unsqueeze(1) > 1).repeat([1, n_max_boxes, 1])
        # 如果有一个anchor被指派去预测多个真实框，首先计算这个anchor最重合的真实框
        # 然后做一个onehot
        # b, 8400
        # 计算每个锚框与哪个真实框的重叠度最高，得到每个锚框应该匹配的真实框索引。
        max_overlaps_idx = overlaps.argmax(1)
        # b, 8400, n_max_boxes
        # 根据max_overlaps_idx构造one-hot编码，表示每个锚框应该匹配的真实框。
        is_max_overlaps = F.one_hot(max_overlaps_idx, n_max_boxes)
        # b, n_max_boxes, 8400
        # 调整is_max_overlaps的形状，并转换为与overlaps相同的类型。
        is_max_overlaps = is_max_overlaps.permute(0, 2, 1).to(overlaps.dtype)
        # b, n_max_boxes, 8400
        # 更新mask_pos，对于被指派给多个真实框的锚框，只保留重叠度最高的那个匹配。
        mask_pos = torch.where(mask_multi_gts, is_max_overlaps, mask_pos)
        # 重新计算fg_mask，即每个锚框是否匹配到真实框。
        fg_mask = mask_pos.sum(-2)
    # 找到每个anchor符合哪个gt
    # 找到每个锚框最终匹配的真实框索引。
...
def xywh2xyxy(x):
    """
    将边界框坐标从（x，y，宽度，高度）格式转换为（x1，y1，x2，y2）格式，其中（x1，y2）是左上角和（x2，y2）是右下角。
    Args：
        x（np.ndarray）或（torch.Tensor）：以（x，y，width，height）格式输入边界框坐标。
    返回：
        y（np.ndarray）或（torch.Tensor）：以（x1，y1，x2，y2）格式表示的边界框坐标。
...
class ModelEMA:
    """ 指数移动平均线（EMA）
保持模型state_dict中所有内容（参数和缓冲区）的移动平均值
有关EMA的详细信息，请参阅https://www.tensorflow.org/api_docs/python/tf/train/ExponentialMovingAverage
    """
